...
cloudjs.define({
    dialog: function(options){
        ...
        var _self = this,
            ...
            CANCLE_TEXT = ['取消', '关闭', 'cancle', 'close', 'no'],
            SPACING = 2,
            LEFT_EDG = 6,
            RIGHT_EDG = 10,
            _dragBinded = false,
        ...
        function _init(){
            ...
            _dialogObj = _dialogObj || _getDialogObj(!!defaults.buttons);
        ...
        function _prompt(content, relative, position, ok, empty){
            ...
            str += ('<textarea class="dialog_div_body_textarea" style="width:' + width + 'px"></textarea>');
         ...
         */
        function _getDialogObj(hasFoot){
            ...
            dialog.mousedown(function(){
            	dialog.css('zIndex', cloudjs.zIndex());
            });
        ...
        function _addButton(name, handler, container, index){
            ...
            for(var i = 0; i < CANCLE_TEXT.length; i++){
            	if(name.toLocaleLowerCase().indexOf(CANCLE_TEXT[i]) >= 0){
    ...
    require: ['../css/' + cloudjs.themes() + '/dialog.css']
});