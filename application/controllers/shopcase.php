...
class Shopcase extends CI_Controller {
	...
	*/
	public function index()
	{
        ...
        $shopcases = $this->shopcase_model->get_shopcases_by_providerid($userid, $length, $offset);
        if (!empty($shopcases))
        {
		...
		$viewdata['ret'] = 0;
		$this->load->view('shopcase_view', $viewdata);
	}
	...
	*/
     ...
     *      image		案例封面/主图片
     *      intro      	案例简介
     *      content    	案例详细内容(展示层自定义内容格式, 使用base64对其进行编码)
     * </pre>
     * @return 操作结果
     * <pre>
     *      {"ret":0,"id":1}
    ...
    function create()
    {
		$case_name       = trim($this->input->get_post('name', TRUE));
		$case_image      = trim($this->input->get_post('image', TRUE));
		$case_intro      = trim($this->input->get_post('intro', TRUE));
        $case_content    = trim($this->input->get_post('content', TRUE));
